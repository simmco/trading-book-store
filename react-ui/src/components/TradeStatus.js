  ...
  render() {
    ...
    return (
      <div>
        ...
        {this.props.trade.requestedBooks &&
          this.props.trade.requestedBooks.map(trade => {
            return trade &&
              <div key={trade._id}>
                <Trade>
                  <TradeInfo>{trade.title} - {trade.authors}</TradeInfo>
                  <TradeDecline onClick={() => this.declineReq(trade._bookId)}>
                    {" "}X
                  </TradeDecline>
                  {" "}
                </Trade>
        ...
        {this.props.books &&
          this.props.books.map(book => {
            return book._requestedBy &&
              <div key={book._id}>
                <Trade>
                  <TradeInfo>{book.title}</TradeInfo>
                  <TradeDecline
                    onClick={() => this.rejectReq(book._id, book._requestedBy)}
                  >
                    {" "}X
                  </TradeDecline>
                  <TradeConfirm>
                    <Link to={{ pathname: `/trade/${book._id}` }}>✔</Link>
                  </TradeConfirm>
                </Trade>
              </div>;
          })}
